    ...
    <div class="py-8">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
                <form action="{{ route('kelas.store') }}" method="POST" class="flex flex-col md:flex-row gap-3">
                    @csrf
                    <input type="text" name="nama" placeholder="Tambah kelas baru" required class="w-full md:flex-1 rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                    <button type="submit" class="px-4 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-700">Tambah Kelas</button>
                </form>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <table class="min-w-full text-left text-sm text-gray-700">
                    ...
                    <tbody>
                            ...
                            <tr class="border-t border-gray-200">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex gap-2">
                                        <form action="{{ route('kelas.update', $item->id) }}" method="POST" class="flex gap-2">
                                            @csrf
                                            @method('PUT')
                                        ...
                                        <form action="{{ route('kelas.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin hapus kelas ini?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-4 py-3 text-gray-500">Belum ada data kelas.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
